...
categories:
    - Oracle
    - Database
metaTitle: "Install Oracle Integration Cloud connectivity agent on an on-premises host"
metaDescription: "To gain access to an on-premises database from the Oracle Integration Cloud, you need to install the Oracle Integration Cloud connectivity agent on an on-premises host."
ogTitle: "Install Oracle Integration Cloud connectivity agent on an on-premises host"
ogDescription: "To gain access to an on-premises database from the Oracle Integration Cloud, you need to install the Oracle Integration Cloud connectivity agent on an on-premises host."
---

To gain access to or to establish connection with an on-premises standalone
...
1. Operating system (O/S) prerequisites include the following:
   a.	Linux&reg; Oracle Enterprise Linux (OEL) version should be either 6 or 7.
...
2.	Java version prerequisites include the following:
   a.	You must install Oracle JDK version 1.7 or 1.8 to install and use the
   connectivity agent.
   b.	Other JDKs are not supported.

3.	Open the port 443 on the on-premises host to enable connectivity between the
...
#### Download the Oracle Integration Cloud connectivity agent
...
The installation in this post used the 64-bit version Java as the Linux version.

**Note:** Move the downloaded java to on-premises host where you want to
install the Connectivity Agent and extract it.

### Install the Integration Cloud connectivity agent

Make sure the installation software that you previously downloaded is already
moved to your your on-premises host server.

Use the following steps to install the connectivity agent in **/u01/app/OICS**
on the on-premises Linux host.

#### 1. Set Java JDK 1.7 or 1.8 to PATH

Extract the latest java JDK version at **/u01/app/OICS** as shown in the following
example:
...
#### 2. Check the connectivity between the on-premises server and the Oracle Integration Cloud

Before installing the agent, verify that the Oracle Integrated Cloud URL can be
accessed from the on-premises server. If it is inaccessible, the connectivity
agent installation will fail. Use the following command, which downloads a small
file, to verify that Oracle Integration Cloud URL is accessible. If the download
...
to location where you want to install the connectivity agent, such as **/u2/app/OICS/Agent**
in this example.
...
The parameters of the command include the following:
...
-	`-u` (Required): The username of the Oracle Integration Cloud Service with
   which agent connects.
-	`-p` (Required): The password of Oracle Integration Cloud Services.
...
-	`-au` (optional): The WebLogic Administrator username, if you don’t want
   WebLogic as a user. You should give username as WebLogic only.
...
Upon successful installation of the agent, the WebLogic console URL as shown in
...
To start the connectivity agent on the on-premises host, set the environments
variable of OICS agent, by using the following commands:
...
on-premises database to Oracle Cloud. The latest version of Oracle integration

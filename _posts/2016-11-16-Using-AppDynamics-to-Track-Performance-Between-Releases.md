...
The process is simple enough; by using cURL we can plug in the appropriate variables and create our API call.  For example, we might use something like this:
...
The Compare Releases functionality is straight forward; all we have to do is pick a time frame before the recorded release time and after the recorded release time to compare.  AppDynamics will allow us to compare performance in broad and granular capacities.

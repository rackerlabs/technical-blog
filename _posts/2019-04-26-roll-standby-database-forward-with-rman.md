...
metaDescription: "This blog shows how you can use the Oracle&reg; Recovery Manager (RMAN) incremental backup to roll the standby database forward faster than by using a redo log apply."
ogTitle: "Roll a standby database forward with RMAN"
ogDescription: "This blog shows how you can use the Oracle&reg; Recovery Manager (RMAN) incremental backup to roll the standby database forward faster than by using a redo log apply."
---

If a physical standby database has lost or corrupted an archived redo data or
has an unresolvable archive gap, which is the result of archive logs getting
missed out on the primary database or the archives getting corrupted, then in
such a case there are no valid backups to recover.

<!-- more -->

If an archive exists on the primary database, troubleshoot the issue, and, if
the archive is not corrupted, the archive is automatically be transferred. If
an archive is corrupted at the operating system (OS) level and you do have a
backup, then restore the databas by using the archive log from the backup. If
...
This blog shows how you can use the Oracle&reg; Recovery Manager (RMAN)
...
standby database’s current SCN, which can then be used to roll forward the standby database.

### Steps to roll forward a physical standby database

Use the following steps to roll forward and physical standby database.
...
#### Take an incremental SCN backup and create standby controlfile backup

1. Run the following code on the production server take an SCN backup:
...
4. Run the following code to apply a incremental SCN backup on the standby
   database, which catalogs all the backup pieces and the control file:
...
7. Shutdown the standby database and mount the standby database so that you can
   mount the standby database with the new control file that was restored in
   the previous step.
...
RMAN incremental backups can be used to synchronize a physical standby database

...
metaDescription: "This blog explains how to move a Microsoft&reg; SQL Server database to Amazon Relational Database Service (RDS)."
...
This blog explains how to move a Microsoft&reg; SQL Server database hosted on
premises (or on Amazon&reg; EC2 or Azure&reg;) to Amazon Relational Database
Service (RDS). This requires backing up your SQL database to AWS S3 bucket and
...
Previous versions of Amazon RDS instance did not let you restore data from
...
to upload the backup file to AWS S3 bucket.
...
-	ProfileName (an AWS user with access to Amazon S3 and S3 bucket including
   the `aws_access_key_id` and `aws_secret_access_key`.)
-	AWS RDS instance with appropriate option group with the `SQLSERVER_BACKUP_RESTORE`
   option mapped to it.
-  PowerShell installation

You also should have basic knowledge about SQL Server, creating an S3 bucket,
creating an AWS user, granting access to S3 bucket, and creating an RDS instance.

### Solution

The solution steps covered in the following sections are:

1.	Backup the database on the local server.

2.	Copy the backup file to an AWS S3 bucket.

3.	Restore the SQL backup in RDS from the S3 bucket.

#### Backup the database on the local server

You can use any method to take backup locally. The following example uses a
**.bat** script with a sqlcmd command that can be used as in any SQL agent job,
...
You can do this by using `AWS S3 copy` or `AWS S3 sync` commands. However, the
...
modified files to the destination. The following PowerShell script can be used
in an SQL agent job. Both the backup and this copy step need to be run by using
...
-	The RDS SQL instance uses the correct option group where the backup/restore
   option has been added. This setting is critical: Without it, restore will not
   begin.
...
-	No differential, transaction log or filegroup backup or restore is possible.
   This is not a hindrance to data recovery. Amazon RDS allows you to create
...
-	The user account running RDS SQL Server native backup and restore commands
...
One more use case of this article is to store your SQL backups directly on AWS

...
metaDescription: "The blog post explains how to integrate Oracle&reg; Application Development Framework (ADF) with E-Business Suite Supply (EBS) Planning Work Area (SWPA)."
...
[Oracle Fusion Middleware Understanding Oracle Application Development Framework](https://docs.oracle.com/middleware/1212/adf/ADFCG/intro.htm#ADFCG111):
...
Oracle describes SPWA in [FAQ For 12.2.5 - SPWA - Supply Planning Work Area](https://support.oracle.com/knowledge/More%20Applications%20and%20Technologies/2049195_1.html):
...
powers Rapid Planning and Fusion applications - part of the Fusion Middle Ware
(FMW) tech stack&mdash;This does not use standard Oracle Forms tech stack that
...
7. Start the admin and managed servers.
8. Deploy the Planning UI application.
9. Set the MSC profile in EBS.

#### 1. Download and install a new or separate WebLogic Server (WLS), version 10.3.6

Download Software from the following URL:
...
2. Make sure that you have JDK 1.6 or later.
...
-	Select the JDK location where JDK you installed the JDK.
...
-	Change the Listener port to other port if you installed the WLS in the same
   EBS Apps Tier.
-  Change to port ``7051``, and if SSL is not enabled, uncheck **SSL**.
`
#### 4. Create a new managed server in WebLogic
...
6. Log in with the WebLogic username and password
...
3. Log in with the WebLogic username and password.
...
<ol start=5>
    ...
    <li>Select ASCPManagedServer from the <b>Target</b> list.</li>
...
4.  Select Deployments in the Domain Structure region, which display the
...
7. Log in with the Weblogic username and password.
...
11. After the deployment completes and you see the PlanningUI deployment,
    select **Deployments** from the **Domain Structure** region and locate
    **PlanningUI** in the **Deployments** table. The PlanningUI deployment
...
you need to install and configure both a WebLogic server and ADF and then enable

...
fields that you shouldn't set, you need to get an understanding of all the keys
...
shouldn't set, you need to get an understanding of all the keys in your MongoDB&reg;
collection.
...
You can get the keys with MapReduce:
...
After the operation finished, run the following `distinct` command on the
resulting collection to find all keys:

     db.activities_keys.distinct("_id")

If you want to get a list of all the unique keys in a subdocument, just modify
the following line:

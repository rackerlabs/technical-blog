---
layout: post
title: "Packet sniffers and how you protect yourself"
...
categories:
  - Security
metaTitle: "Packet sniffers and how you protect yourself"
metaDescription: "This post explores packet sniffers, which are useful tool and a potential threat."
ogTitle: "Packet sniffers and how you protect yourself"
ogDescription: "This post explores packet sniffers, which are useful tool and a potential threat."
---

This post explores packet sniffers, which are useful tool and a potential
threat, and provides detailed information about packet sniffers, software used
...
A combination of hardware and software tools which can detect, decode, and
manipulate traffic on the network. Network administrators use this to
troubleshoot networking issues, but many hackers use it to gather vital
information.
...
networks breaks down the data to be transmitted into several packets. They then
...
-	TSHARK offers command line interface and has the following three components:
   -	**Editcap**: Reads the captured packets from the infile, reads and writes
   ...
   -	**Text2pcap**: Reads in an ASCII hex dump and writes the data described
      into a `pcap` or `pcapng` capture file. Text2pcap can read hexdumps with
...
Wireshark is very popular network analyzer tool, which is used by network
...
In a switched environment a host only receives:
...
ARP poisoning is when an attacker sends falsified ARP messages over a local
area network (LAN) to link an attacker’s MAC address with an IP address of a
legitimate computer or a server on the network. After the attacker’s MAC address
...
–	Sensitive, unencrypted communications
   -	Web requests or replies, e-mail, FTP, HTTP.
...
Use the following techniques and best practises to protect yourself from
sniffing attacks:

...
store which captures database history of executed queries, query runtime
...
SQL Server restarts to not affect the data. Use Query Store, which was introduced
...
-	**Operation Mode (Requested)** has three values: `Off`, `Read Only` and
   ...
   execution plans or query runtime statistics are collected. This is for
...
-  **Max Size (MB)** is used to configure the maximum size of the Query Store.
   ...
   configured here, after Query sSore reaches the maximum size, the operation
...
-  **Capture Mode** helps you choose which type of query to capture in Query
   ...
   by priority, and tries to ignore infrequently executed and other ad hoc queries.
...
-	**Top Resource Consuming Queries**: Queries which have biggest impact to
...
-	**Tracked Queries**: Track the execution of most important queries in real
   time.
...
with message “Do you want to force plan 216 for query 42?”
...
-	Use latest SQL Server Management Studio to see latest features and enhancements.
...
performance enhancements is one of key skills required for every database
...
query plan for any query. This will override the one saved in the plan cache,

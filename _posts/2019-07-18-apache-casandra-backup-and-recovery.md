...
metaDescription: "This blog shows you how to backup Cassandra databases and restore them after a failure."
...
This blog shows you how to backup Cassandra databases and restore them after a
failure.
...
Though Apache&reg; Cassandra&reg; is decentralized, it can endure single and
...
successfully manage the manage large volume of business data, more popularly
...
you must use the `sstableloader` utility, which not only copies the set of
sstables to each node but also transfers the appropriate part of the data to
every node based on the replication strategy defined for a cluster. Note that
it is not necessary to have an empty table to restore the data.
...
Create a softlink for the user directory with the name of the table that you
...
**schema.cql** with backup that contains the data definition language (DDL) of
the objects that reside in **keyspace**.

Use **schema.cql** to create employee object, which got dropped by mistake.
...
transfers the relevant part of data to each node based on replication strategy
...
preceding steps without the table restore part. You are must recreate the
**keyspace** and load data by using `sstableloader`.

The number of nodes in the source and target databases cluster don’t matter with
`sstableloader` because it reads each **sstables** from the backup and streams
the data into cluster while placing the data as per the defined replication
